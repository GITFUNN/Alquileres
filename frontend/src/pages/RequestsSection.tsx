    ...
    const fetchData = async () => {
      try {
        const enrichedData = await Promise.all(
          data?.map(async (request: Requests) => {
            const senderEmailData = await getSenderEmail(request.sender);
            const senderEmail = senderEmailData?.email;
            const apartmentDetailsData = await getApartmentNumber(
              request.apartment
            );
            const apartmentDetails = apartmentDetailsData?.number;
            const condondominiumDetailsData = await getCondominiumName(
              request.condominium
            );
            const condominiumDetails =
              condondominiumDetailsData?.condominium_name;
    ...
    <div className="flex justify-center">
      <div className="mx-auto my-auto w-11/12">
        {requests.map((request) => (
          <div
            key={request.id}
            className="w-11/12 sm:py-4 sm:w-[500px] bg-white text-black justify-center my-6 sm:my-2 mx-auto"
          >
            <div className="rounded-lg border-black border">
              <div className="flex justify-center items-center">
                <img className="w-6 h-6" src={jjrequest} alt="jrequest" />
                <span className="px-4 py-3 text-center font-medium text-lg ">
                  Condominium Request
                </span>
              </div>
              <p className="px-4 py-3 selection:bg-indigo-500">
                {" "}
                <TruncateDate timestamp={request.timestamp} />
              </p>
              <p className="px-4 py-3 selection:bg-indigo-500">
                Dear Resident,
              </p>
              <p className="px-4 py-3 mb-5 selection:bg-indigo-500">
                You are hereby granted access to the condominium{" "}
                <a className="underline decoration-indigo-500 font-medium">
                  {request.condominiumDetails}
                </a>{" "}
                by the owner{" "}
                <a className="underline decoration-indigo-500 font-medium">
                  {request.senderEmail}
                </a>
                . This access is valid for the following apartment:{" "}
                <a className="underline decoration-indigo-500 font-medium">
                  {request.apartmentDetails}
                </a>
              </p>
              <SetRequestState
                ApId={request.apartment}
